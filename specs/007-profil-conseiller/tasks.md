...
- [ ] T003 [P] Créer `packages/db/prisma/schema/profil.prisma` skeleton vide (multi-file Prisma — vérifier que `previewFeatures = ["prismaSchemaFolder"]` est actif côté `schema.prisma` racine ; si non, l'activer)
...
- [ ] T008 Migration Prisma `20260527_create_profil_tables` : créer enums `StatutProfil`, `OnboardingRelanceEtape`, `OnboardingRelanceEtat`, `ProfilModerationAction`, `PhotoUploadStatut` + tables `conseiller_profiles`, `profile_photo_history`, `slug_reservations`, `profile_onboarding_reminder_schedules`, `profil_moderation_audits`, `profile_specialities`, `profile_geo_zones`, `profile_languages` + index + triggers (`prevent_slug_mutation_after_publish`, `prevent_unanonymize`, append-only `slug_reservations`, append-only `profil_moderation_audits`) + check constraint `chk_raison_masquage_coherence` (cf. data-model.md)
- [ ] T009 Migration Prisma `20260527_seed_profil_enums` : INSERT initial des codes FR-CA dans `profile_specialities` (12), `profile_geo_zones` (12), `profile_languages` (6) — cf. data-model.md Migration 2
- [ ] T010a **Pré-investigation 001** : lire `specs/001-conformite-module/data-model.md` + `packages/db/prisma/schema/conformite.prisma` (ou équivalent existant). Confirmer présence/absence des colonnes `prenom_legal` et `nom_legal` (ou variantes nommées différemment) dans `dossiers_conformite`. Documenter le résultat dans un commentaire en tête de T010 ; si les colonnes existent sous d'autres noms, mettre à jour T043 (adaptateur `PrismaNomLegalReader`) pour utiliser ces noms et supprimer T010
- [ ] T010 Migration Prisma `20260527_extend_dossier_conformite_with_legal_names` (conditionnelle suite T010a) : `ALTER TABLE dossiers_conformite ADD COLUMN prenom_legal VARCHAR(80), nom_legal VARCHAR(80)` **uniquement si absent** d'après T010a ; sinon supprimer cette migration et noter dans le runbook (cf. data-model.md Migration 3)
...
- [ ] T033 [P] Créer le port `apps/api/src/modules/identite/application/ports/conformite-nom-legal-reader.port.ts` avec interface `ConformiteNomLegalReader` + type `NomLegal` (cf. contracts/conformite-nom-legal.port.md)
...
- [ ] T043 [P] Implémenter `apps/api/src/modules/conformite/infrastructure/prisma-nom-legal-reader.ts` (impl du port T033, lit `dossier_conformite.prenom_legal` + `nom_legal` + statut)
...
- [ ] T047 Créer `apps/api/src/modules/conformite/conformite.module.ts` exports `ConformiteNomLegalReader` (extension exports module existant 001)
...
- [ ] T051 [P] Étendre `tools/check-module-boundaries.ts` (existant 006) pour ajouter les règles : `packages/profil-domain/` ne peut PAS importer NestJS / Prisma / Next.js / Auth.js / `@aws-sdk/*` ; `apps/api/src/modules/identite/application/` ne peut PAS importer Prisma ni `@aws-sdk/*` directement
...
- [ ] T060a [US1] **Pré-investigation event 001** : vérifier l'existence et la signature de `ConseillerConformiteChangedEvent` côté module conformité (`apps/api/src/modules/conformite/domain/events/` ou équivalent). Documenter : nom exact, payload (`{conseillerId, ancienStatut, nouveauStatut, occurredAt}` attendu), mécanisme de diffusion (NestJS `EventEmitter2`, BullMQ pub/sub, ou outbox pattern). **Si absent** : ouvrir un PR préparatoire côté 001 pour l'émettre ou inclure son émission dans le PR courant. **Bloque T061.**
- [ ] T061 [US1] Listener `apps/api/src/modules/identite/application/listeners/conformite-changed.listener.ts` souscrit à `ConseillerConformiteChangedEvent` (confirmé par T060a) : `pending → verified` → crée `ConseillerProfile` si inexistant (statut `incomplet`) + appelle `PlanifierRelancesOnboardingUseCase` (J+3/7/14) ; `verified → expired/revoked` → recalcule statut profil + invalidations Next.js + CloudFront. **Depends on T060a, T036, T041, T093.**
- [ ] T061a [US1] **[TDD RED + GREEN]** Tests intégration listener `apps/api/test/integration/profil/conformite-changed-listener.spec.ts` couvrant : (a) `pending → verified` 1re fois → crée profil + planifie 3 jobs BullMQ + statut `incomplet` ; (b) `pending → verified` profil existant déjà incomplet → idempotent (pas de double création, pas de double planification grâce au `jobId` déterministe) ; (c) `verified → expired` → recalcule statut → invalidations Next.js + CloudFront appelées ; (d) `expired → verified` (re-vérification) profil au statut `incomplet` → **les relances NE sont PAS re-déclenchées** (edge case spec « Re-vérification après expiration ») ; (e) `expired → verified` profil au statut `pret` → page publique redevient accessible ≤ 60 s, aucune relance émise
- [ ] T062 [P] [US1] **[TDD GREEN sur intégration]** S'assurer que T053-T056 + T061a passent au vert après T057-T061
- [ ] T063 [P] [US1] Server Action `apps/web/src/app/(conseiller)/conseiller/profil/_actions/editer-profil.ts` consommant `EditerProfilUseCase` via fetch API ; utilise `useActionState` côté composant
- [ ] T064 [P] [US1] Server Action `apps/web/src/app/(conseiller)/conseiller/profil/_actions/uploader-photo.ts` (multipart) consommant `UploaderPhotoUseCase`
- [ ] T065 [US1] Page `apps/web/src/app/(conseiller)/conseiller/profil/page.tsx` (RSC) : chargement initial via `LireProfilPriveUseCase`, formulaire client `<ProfilForm>`
- [ ] T066 [P] [US1] Composant `apps/web/src/app/(conseiller)/conseiller/profil/_components/profil-form.tsx` (client) avec `react-hook-form` + `@hookform/resolvers/zod` + shadcn `<Form>` (intègre tous les champs ; toggles `afficherNomComplet` avec avertissement FR-006b)
- [ ] T067 [P] [US1] Composant `apps/web/src/app/(conseiller)/conseiller/profil/_components/photo-upload.tsx` (client) avec `<input type="file">` + preview + progress + erreur Result
- [ ] T068 [P] [US1] Composant `apps/web/src/app/(conseiller)/conseiller/profil/_components/multi-select-field.tsx` (combobox accessible, role="listbox", aria-multiselectable) pour spécialités/langues/zones
- [ ] T069 [P] [US1] Composant `apps/web/src/app/(conseiller)/conseiller/profil/_components/afficher-nom-complet-switch.tsx` avec avertissement FR-006b (texte d'aide + lien `/politique-loi25`)
...
- [ ] T078 [US2] **[TDD GREEN]** Implémenter `apps/api/src/modules/identite/application/use-cases/lire-page-profil-publique.use-case.ts` (consomme `ProfilPublicReader` + `ConformiteQueryPort.estVerifie` + `ConformiteQueryPort.certifications`) retournant `ProfilPublicPayload | null` (cf. contracts/profil-public.port.md)
...
- [ ] T080 [US2] **[TDD GREEN]** Page Next.js `apps/web/src/app/conseiller/[slug]/page.tsx` (RSC) avec `params: Promise<{slug: string}>`, `export const experimental_ppr = true`, `export const dynamicParams = true`, `generateStaticParams = async () => []`, `revalidate = 300` (cf. plan.md L1+L3 + research.md R4-bis)
- [ ] T081 [US2] **[TDD GREEN]** Si profil retourné `null` → `notFound()` Next.js → `apps/web/src/app/not-found.tsx` partagé (statique, constant body) — créer ou étendre cette page racine (cf. research.md R7 + C2)
- [ ] T082 [P] [US2] Composant `apps/web/src/app/conseiller/[slug]/_components/profil-hero.tsx` (photo `next/image` `priority`, nom affiché, titre)
- [ ] T083 [P] [US2] Composant `apps/web/src/app/conseiller/[slug]/_components/profil-sections.tsx` (biographie, spécialités, langues, zones, années d'expérience)
- [ ] T084 [P] [US2] Composant `apps/web/src/app/conseiller/[slug]/_components/certifications-list.tsx` (lue depuis le payload, badges CCV/TICO)
- [ ] T085 [P] [US2] Composant `apps/web/src/app/conseiller/[slug]/_components/section-pourquoi-pas-contact.tsx` (encart pédagogique FR-009 avec lien `/comment-ca-marche`)
- [ ] T086 [P] [US2] Composant `apps/web/src/app/conseiller/[slug]/_components/cta-suggested.tsx` (lien unique `<a href="/intake?suggested=<conseillerId>">`)
- [ ] T087 [US2] Page `apps/web/src/app/conseiller/[slug]/opengraph-image.tsx` (génération OG image dynamique avec nom affiché + spécialité principale)
- [ ] T088 [US2] Métadonnées Next.js dans `apps/web/src/app/conseiller/[slug]/page.tsx` : `export async function generateMetadata({params})` retournant `title`, `description`, `openGraph`, `twitter`, `canonical`, JSON-LD `Person` minimal sans `contactPoint`/`telephone` (cf. plan.md Principe XII)
- [ ] T089 [US2] **Middleware Next.js** `apps/web/src/middleware.ts` étendu (cf. contracts/intake-suggested-middleware.md) : si requête `/intake?suggested=<id>` → validation UUID v4 → décoder cookie existant `cv_suggested` (HMAC) → append entry FIFO ≤ 10 → ré-encoder + signer → `Set-Cookie` HttpOnly + SameSite=Lax + Path=/intake + Max-Age=86400 → redirect 302 `/intake`. Chaîné AVEC auth + CGU existants. **Depends on T023 (suggested-cookie HMAC), T006 (secret env), middleware auth + CGU existants 002a/004.**
...
- [ ] T091 [US2] Route Next.js `apps/web/src/app/sitemap.xml/route.ts` (ou `sitemap.ts`) : appelle `ProfilPublicReader.lireSlugsPubliables()` → XML standard ; `revalidate = 3600` ; cf. research.md R4-bis pour pagination future
...
- [ ] T098 [US3] Page Server Component `apps/web/src/app/(conseiller)/conseiller/page.tsx` consomme `LireProfilPriveUseCase` + ConformiteQueryPort (date expiration)
- [ ] T099 [P] [US3] Composant `apps/web/src/app/(conseiller)/conseiller/_components/widget-conformite.tsx`
- [ ] T100 [P] [US3] Composant `apps/web/src/app/(conseiller)/conseiller/_components/widget-profil.tsx` (statut + avertissement FR-012a)
- [ ] T101 [P] [US3] Composant `apps/web/src/app/(conseiller)/conseiller/_components/widget-leads-placeholder.tsx` (« Bientôt disponible — feature 012 »)
- [ ] T102 [P] [US3] Composant `apps/web/src/app/(conseiller)/conseiller/_components/widget-facturation-placeholder.tsx` (« Bientôt disponible — feature 006-007 »)
...
- [ ] T108 [US4] Page `apps/web/src/app/(conseiller)/conseiller/profil/apercu/page.tsx` réutilise les composants de US2 (`profil-hero`, `profil-sections`, etc.) + ajoute `BandeauApercu` si non publié
- [ ] T109 [P] [US4] Composant `apps/web/src/app/(conseiller)/conseiller/profil/apercu/_components/bandeau-apercu.tsx` (alert jaune avec liste champs manquants ou raison masquage)
...
- [ ] T121 [P] [US6] Page `apps/web/src/app/(admin)/admin/profils/page.tsx` (liste + filtres statut + recherche par nom légal/slug, intégrée à la console conformité existante via tabs)
- [ ] T122 [P] [US6] Page `apps/web/src/app/(admin)/admin/profils/[id]/page.tsx` (détail profil + historique modérations + actions)
- [ ] T123 [P] [US6] Composant `apps/web/src/app/(admin)/admin/profils/_components/dialog-confirmation-action.tsx` (Radix Dialog + textarea raison + focus trap)
