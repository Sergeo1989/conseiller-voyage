...
- [x] T019 [US1] Sweep de réconciliation (pattern 012) dans `infrastructure/jobs/enrichment-reconciliation.sweep.ts` : brief `activated` non apparié sous N min → appariement (filet anti-perte de job)
...
- [x] T027 [US3] Idempotence : court-circuit dans `EnrichBriefUseCase` si `BriefEnrichment` existe pour `briefId` (0 appel) ; gère la livraison **at-least-once** concurrente (contrainte unique T002 + court-circuit)
- [x] T028 [US3] Maîtrise du coût : `maxOutputTokens` + troncature du texte d'entrée (≤ 0,05 USD/req, Principe V) dans `EnrichBriefUseCase` / l'appel `LlmProvider`
- [x] T029 [US3] Métriques OTel `cv.intake.enrichment.*` (attempts / success / fallback **par cause** / latency / tokens ; option : taux de détection PII pré-scrub)
...
- [x] T036 README enrichissement (module intake) + runbook ops + ADR-0028 → *accepté* + `quickstart.md` § Statut de validation (SC-001→SC-009) + DoD cochée
