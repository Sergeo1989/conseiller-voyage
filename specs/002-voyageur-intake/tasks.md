...
- [X] T011 Définir dans `packages/db/prisma/schema/intake.prisma` les 6 enums : `BriefStatus`, `TravelBudget`, `TravelSpeciality`, `TravelFamiliarity`, `ConseillerLanguage`, `MagicLinkPurpose` (mapping data-model.md §enums) — **note exécution : `ActorRole` réutilisé du module conformité (001), pas redéfini**
- [X] T012 Définir dans `packages/db/prisma/schema/intake.prisma` les 5 modèles : `VoyageurContact`, `VoyageurBrief`, `MagicLinkToken`, `IntakeAuditEntry`, `IntakeOutboxEntry` (tous champs/indexes/FK du data-model.md) — **N4 résolu : `emailHashAfterErasure` ajoutée explicitement au tableau VoyageurContact ; N9 résolu : `abuseMarkedAt` ajoutée explicitement à VoyageurBrief**
- [X] T013 Créer migration `packages/db/prisma/migrations/20260528170001_init_intake/migration.sql` : tables + index uniques (`email`, `idempotency_key WHERE NOT NULL`, `token_hash`) — **SQL généré via `prisma migrate diff --from-empty` et filtré ; pas `prisma migrate dev` car Docker pas lancé en local — la migration sera appliquée au prochain `pnpm db:migrate` après `pnpm docker:up`**
- [X] T014 Créer migration `packages/db/prisma/migrations/20260528170002_intake_audit_append_only/migration.sql` : trigger Postgres `intake_audit_block_modifications` (copie/adapter de `conformite_audit_block_modifications`) bloquant UPDATE/DELETE — **étendu : trigger TRUNCATE statement-level (leçon 001) + rôle `app_intake` créé conditionnellement + grants least-privilege + lectures cross-module limitées (auth_users, conformite_conseiller_compliances)**
- [X] T015 Créer migration `packages/db/prisma/migrations/20260528170003_intake_anonymisation_trigger/migration.sql` : trigger Postgres sur `voyageur_contacts` qui rejette tout UPDATE de `firstName/lastName/phone/postalCode` en NON-NULL après `anonymizedAt IS NOT NULL` (idempotence anonymisation Loi 25, pattern hérité de 001/007) — **étendu : email aussi (anti-réintroduction PII) + status `anonymized` terminal sur VoyageurBrief**
- [X] T016 `pnpm --filter @cv/db prisma generate` puis `pnpm --filter @cv/db migrate dev --name init_intake` en local LocalStack — vérifier que `prisma.voyageurBrief` est typé — **`generate` OK : les 5 modèles intake sont exposés (voyageurBrief, voyageurContact, magicLinkToken, intakeAuditEntry, intakeOutboxEntry). `migrate dev` reporté à la prochaine session avec Docker up**
