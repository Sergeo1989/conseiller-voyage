...
- [X] T093 ✅ **PR satellite Mode B** (ADR-0024 §E3) — branche `011-matching-outbox-publisher`. Plutôt que faire le module notifications/conformité réécrire dans les tables matching (violation Principe V), un **outbox publisher appartenant au module matching** draine `matching_outbox_entries` → bus Redis : port `MatchingEventPublisher` + adapter `RedisMatchingEventPublisher` (canal `MATCHING_PUBSUB_CHANNEL`, défaut `matching.events`) + `MatchingOutboxPublisherJob` (scan non-publiés → publish nom kebab-case via `toEventBusName` → `publishedAt`) + scheduling `OnModuleInit` (5 s prod / 30 s dev). Idempotence at-least-once via `idempotencyKey` propagée aux consommateurs (012). Test d'intégration Testcontainers (drain → bus → publishedAt + mapping enum→kebab + pas de republish). Le wiring du trigger entrant intake→bus (`voyageur.brief.activated`) reste un sujet distinct (hors T093).
