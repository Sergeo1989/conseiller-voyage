...
- [ ] T081c [P] [US1] **Test attribution admin nominatif FR-018 / U3 du review** : test unitaire `PrismaAuditLogWriter` qui rejette toute entrée avec `actorRole === 'admin'` mais `actorId === null`. Garantit la traçabilité opérationnelle — `apps/api/src/modules/conformite/infrastructure/__tests__/prisma-audit-log-writer.test.ts`
